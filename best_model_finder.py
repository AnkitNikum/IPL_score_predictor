...
class Model_Finder:
    ...
    def __init__(self,file_object,logger_object):
        ...
        self.xgbreg = GradientBoostingRegressor()
    ...
    def get_best_params_for_XGB_Regressor(self, train_x, train_y):
        """
                                                Method Name: get_best_params_for_Random_Forest_Regressor
                                                Description: get the parameters for Random_Forest_Regressor Algorithm which give the best accuracy.
                                                             Use Hyper Parameter Tuning.
    ...
    "loss": ['squared_error', 'absolute_error'],
            ...
            # Creating an object of the Grid Search class
            self.grid = GridSearchCV(self.xgbreg, self.param_grid_xgb, verbose=3, cv=5)
            # finding the best parameters
            self.grid.fit(train_x, train_y)

            # extracting the best parameters
            self.n_estimators_xgb = self.grid.best_params_['n_estimators']
            self.max_depth_xgb = self.grid.best_params_["max_depth"]
            self.min_samples_split_xgb = self.grid.best_params_['min_samples_split']
            self.learning_rate = self.grid.best_params_["learning_rate"]
            self.loss = self.grid.best_params_["loss"]

            # creating a new model with the best parameters
            self.xgb_regressor = GradientBoostingRegressor(n_estimators=self.n_estimators_xgb, max_depth=self.max_depth_xgb,
                                                         min_samples_split=self.min_samples_split_xgb, learning_rate=self.learning_rate,loss = self.loss)
            # training the mew models
            self.xgb_regressor.fit(train_x, train_y)
            self.logger_object.log(self.file_object,
                                   'XGB reg best params: ' + str(
                                       self.grid.best_params_) + '. Exited the RandomForestReg method of the Model_Finder class')
            return self.xgb_regressor
        except Exception as e:
            self.logger_object.log(self.file_object,
                                   'Exception occured in XGB reg method of the Model_Finder class. Exception message:  ' + str(
                                       e))
            self.logger_object.log(self.file_object,
                                   'XGB reg Parameter tuning  failed. Exited the knn method of the Model_Finder class')
            raise Exception()
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        # create best model for XGBoost
            self.xgb_regressor = self.get_best_params_for_XGB_Regressor(train_x, train_y)
            self.prediction_xgbReg = self.xgb_regressor.predict(test_x)  # Predictions using the randomForestReg Model
            self.prediction_xgbReg_error = r2_score(test_y,self.prediction_xgbReg)


            #comparing the two models
            dict1 = {'xgb':self.prediction_xgbReg_error,'lin': self.LinearReg_error,'rf':self.prediction_randomForestReg_error}
            best_m = min(dict1, key=dict1.get)
            if best_m == 'rf':
                return 'RandomForestRegressor',self.randomForestReg
            elif best_m == 'xgb':
                return 'RandomForestRegressor',self.xgb_regressor
            else:
